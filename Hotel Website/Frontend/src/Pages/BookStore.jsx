...
function BookStore() {
  ...
  const makePayment = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:4500/api/ARZ/booking/checkout', {
        roomData: roomData,
  ...
  useEffect(() => {
    ...
    const saveBooking = async () => {
      try {
        const response = await axios.post('http://localhost:4500/api/ARZ/booking', { BookRoomdata: BookRoomData });
