...
const OffcanComponent = ({ showIn }) => {
  ...
  const makePayment = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:4500/api/ARZ/giftcard/checkout', { Gifts: giftsData, Quantity: totalQuantity });
  ...
  useEffect(() => {
    const savingGift = async () => {
      try {
        const response = await axios.put(`http://localhost:4500/api/ARZ/user/${user._id}`, { giftStore: Gift});
