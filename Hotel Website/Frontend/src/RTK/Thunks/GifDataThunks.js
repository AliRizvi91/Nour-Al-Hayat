...
export const getAllGift = createAsyncThunk(
    ...
    async () => {
      try {
        const response = await axios.get('http://localhost:4500/api/ARZ/giftcard');
...
export const postGift = createAsyncThunk(
    ...
    async ({ formData }) => {
      ...
      try {
        ...
        const response = await axios.post('http://localhost:4500/api/ARZ/giftcard', formData, {
          headers: {
...
export const DeleteGift = createAsyncThunk(
    ...
    async ({_id}) => {
      try {
        ...
        // Ensure the request is sent with the correct content-type for form data
        const response = await axios.delete(`http://localhost:4500/api/ARZ/giftcard/${_id}`);
      ...
      console.log("formData",formData);
        try {
            ...
            const response = await axios.put(
                `http://localhost:4500/api/ARZ/giftcard/${Id}`,formData
            );
