...
export const getAllBooking = createAsyncThunk(
    ...
    async () => {
      try {
        const response = await axios.get('http://localhost:4500/api/ARZ/booking');
...
export const postBooking = createAsyncThunk(
    ...
    async ({ formData }) => {
      ...
      try {
        ...
        const response = await axios.post('http://localhost:4500/api/ARZ/booking', formData, {
          headers: {
...
export const DeleteBooking = createAsyncThunk(
    ...
    async ({_id}) => {
      try {
        // Ensure the request is sent with the correct content-type for form data
        const response = await axios.delete(`http://localhost:4500/api/ARZ/booking/${_id}`);
    ...
    async ({ Id, formData }, { rejectWithValue }) => {
        try {
            ...
            const response = await axios.put(
                `http://localhost:4500/api/ARZ/booking/${Id}`,formData
            );
