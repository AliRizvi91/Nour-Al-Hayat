...
export const getAllRoom = createAsyncThunk(
    ...
    async () => {
      try {
        const response = await axios.get('http://localhost:4500/api/ARZ/room');
...
export const postRoom = createAsyncThunk(
    ...
    async ({ formData }) => {
      try {
        ...
        const response = await axios.post('http://localhost:4500/api/ARZ/room', formData, {
          headers: {
...
export const DeleteRoom = createAsyncThunk(
    ...
    async ({_id}) => {
      try {
        ...
        // Ensure the request is sent with the correct content-type for form data
        const response = await axios.delete(`http://localhost:4500/api/ARZ/room/${_id}`);
    ...
    async ({ Id, formData }, { rejectWithValue }) => {
        try {
            ...
            const response = await axios.put(
                `http://localhost:4500/api/ARZ/room/${Id}`,formData
            );
